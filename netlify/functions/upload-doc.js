...
На примере:
- В пятом пункте удостоверения есть строка вроде "AF......." — это СЕРИЯ+НОМЕР ВУ целиком.
...
Нужно:
- "license_full" — полный код из нижней строчки типа "AF......." (вместе буквы+цифры, без пробелов).
- "license_series" — ТОТ ЖЕ полный код серии, что и в "license_full" (буквы+цифры, как на документе).
- "license_number" — только цифровая часть из этого же кода, если её можно отделить (например "000488684"), иначе null.
...
ВАЖНО:
- Серия/номер ВУ берутся только из строки с кодом типа "AF.......". Серые большие цифры на фоне, водяные знаки, коды UZ и т.п. игнорируй.
- Если серия/номер не видно — ставь null.
`
      : docType === "tech_front"
      ? `
...
Нужно:
- "plate_number" — госномер из пункта 1 (например "01Y984SB").
...
- "pinfl" — ПИНФЛ владельца из пункта 8 (строка цифр) или null, если его нет.
...
- Отдельное поле "Серия" с маленьким чёрным буквенно-цифровым кодом находится в верхнем левом углу оборота (иногда рядом с текстом, отличающимся от основного набора полей).
- Рядом с ним или чуть правее есть поле "№" — номер техпаспорта.
- СЕРИЯ ВСЕГДА БУКВЫ+ЦИФРЫ (например "AAC0554997" или "AAF4222435") и пишется мелким чёрным шрифтом.
- Большие серые цифры, водяные знаки и штампы никогда не являются серией и номером техпаспорта.

Нужно:
- "tech_series" — вся серия техпаспорта ЦЕЛИКОМ (буквы+цифры из поля "Серия", без пробелов). Если серия и номер слиты в одно поле — запиши всё целиком сюда.
- "tech_number" — номер техпаспорта из поля "№", если он выделен отдельно, иначе null.
- "tech_full" — серия+номер техпаспорта так, как они написаны на документе (например "AAC0554997").
...
- Если какого-то поля точно не видно, ставь null.
`;
...

{
  ...
  "fields": {
    // для vu_front:
    "license_series": "ВЕСЬ буквенно-цифровой код серии ВУ как на документе (например \\"AF.......\\") или null",
    "license_number": "Только цифровая часть номера ВУ (например \\"000488684\\") или null, если отделить нельзя",
    "license_full": "Полный код серии+номера ВУ как на документе (обычно совпадает с license_series) или null",
    ...
    // для tech_back:
    "tech_series": "серия техпаспорта (буквы+цифры из поля \\"Серия\\", НЕ госномер, НЕ PINFL, НЕ номер кузова) или null",
...
Правила:
- Если информации нет или её не видно — ставь null.
